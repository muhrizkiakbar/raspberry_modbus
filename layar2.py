...
font_title = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 26)
font_label = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 20)
font_value = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 24)
...
font_time = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 18)
font_countdown = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 16)
...
sensors = [
    {"name": "pH", "value": 7.0, "unit": "", "min": 0, "max": 14, "warn_min": 6.5, "warn_max": 8.5},
    {"name": "TDS", "value": 500, "unit": "ppm", "min": 0, "max": 1000, "warn_min": 300, "warn_max": 700},
    {"name": "TSS", "value": 100, "unit": "mg/L", "min": 0, "max": 500, "warn_min": 50, "warn_max": 200},
    {"name": "Water Level", "value": 75, "unit": "%", "min": 0, "max": 100, "warn_min": 20, "warn_max": 90},
...
def display_sensor_page(page_number, time_left):
    with canvas(device) as draw:
        ...
        draw.text((480//2 - title_width//2, 25), title, font=font_title, fill=TEXT_COLOR)
        ...
        if num_sensors == 1:
            ...
            draw.text((x + 20, y + 20), page_sensors[0]["name"], 
            ...
            value_font = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 36)
            value_width = draw.textlength(value_text, font=value_font)
            draw.text((x + panel_width//2 - value_width//2, y + 70), value_text, 
            ...
            draw.text((x + panel_width - 30 - draw.textlength(max_text, font=font_label), y + 140), 
                     max_text, font=font_label, fill=TEXT_COLOR)
            ...
            for i, sensor in enumerate(page_sensors):
                ...
                draw_rounded_panel(draw, x, y, panel_width, panel_height, 8, PANEL_COLOR)
                ...
                draw.text((x + panel_width - value_width - 10, y + 10), 
                         value_text, font=font_value, fill=value_color)
                
                # Tampilkan satuan
                draw.text((x + panel_width - draw.textlength(sensor["unit"], font=font_unit) - 10, 
                          y + 40), sensor["unit"], font=font_unit, fill=(180, 180, 200))
        ...
        draw.text((20, footer_y + footer_height//2), page_text, font=font_page, fill=TEXT_COLOR, anchor="lm")
        
        # Tampilkan waktu saat ini
        time_text = time.strftime("%H:%M:%S")
        time_width = draw.textlength(time_text, font=font_time)
        draw.text((480 - 20, footer_y + footer_height//2), time_text, font=font_time, fill=TEXT_COLOR, anchor="rm")
        ...
        for sensor in sensors:
            if sensor["value"] < sensor["warn_min"] or sensor["value"] > sensor["warn_max"]:
                ...
                break
        draw.ellipse((480//2 - 15, footer_y + 7, 480//2 + 15, footer_y + 23), fill=status_color)
        ...
        draw.text((480//2 - countdown_width//2, footer_y + footer_height//2), 
                 countdown_text, font=font_countdown, fill=(200, 200, 100), anchor="lm")
